...
const CalendarComponent = () => {
  ...
  const filteredResults = data?.result
    .filter(
      (res: { status: string; bookingDate?: { startDate?: string } }) =>
        res.status === "successful" &&
        res.bookingDate?.startDate &&
        res.bookingDate.startDate >= currentDateStr
    )
    .sort(
      (a: UpcomingOutingProps, b: UpcomingOutingProps) =>
        new Date(a.bookingDate?.startDate ?? 0).getTime() -
        new Date(b.bookingDate?.startDate ?? 0).getTime()
    );
  ...
  return (
    <div className={styles.container}>
      ...
      {isSuccess && filteredResults.length > 0 ? (
        <div>
          <Text className="py-3 pl-2 font-dmSansBold text-[18px] font-bold">
            Next Schedules
          </Text>
          {filteredResults.map((info: UpcomingOutingProps) => (
            <div
              key={info.id}
            ...
            >
              ...
              />
              <div>
                <p className="text-[14px] font-semibold">{info.outing?.name}</p>
